...
# v00
#   - Original Python API provided by Zephyr

# v01
#   - Customized API to receive trigger from a server in a UDP socket
#   - Writes specific packets of data in a .txt file

# v02 (current version)
#   - Improved code-readability and efficiency
#   - Streaming of data to a server with UDP
...
def parse_timestamp(encoded):
    ...
    str_stmpyear = f'{year:04d}'
    str_stmpmonth = f'{month:02d}'
    str_stmpday = f'{day:02d}'
    hour, dec = divmod(msec/3600/1000, 1)
    hour = int(hour)
    str_hour = f'{hour:02d}'
    minute, dec = divmod(dec*60, 1)
    minute = int(minute)
    str_minute = f'{minute:02d}'
    second, dec = divmod(dec*60, 1)
    second = int(second)
    str_second = f'{second:02d}'
    dec = round(dec*1000)
    str_msec = f'{dec:03d}'

    # Francesco 02.02.22
    global str_timestamp
    str_timestamp = (str_hour + ":" + str_minute + ":" + str_second + "." + str_msec + "\t")   
    return stamp
...
class SummaryDataMessageV3(SummaryDataMessage):
    ...
    def __init__(self, msgid, payload, fin=MC.ETX):
        ...
        # Francesco
        global udpPacket_summary
        str_HR = repr(self.heart_rate)
        str_RR = repr(self.respiration_rate)
        #udpPacket_summary = trigger + "\t" + str_timestamp + "\t" + str_HR + "\t" + str_RR + '\n' # string to locally write on file
        #udpPacket_summary = trigger + "\t" + str_timestamp + "\t" + str_HR + "\t" + str_RR + '\t' #string to stream
        udpPacket_summary = str_HR + "\t" + str_RR + '\t' #string to stream

        # global udpPacket
        # global trigger
        # global isSynchDone
        # udpPacket = trigger + "\t" + str_timestamp + "\t" + udpPacket_summary + "\t" + udpPacket_ecg + '\t' + udpPacket_breathing + '\n'
        
        # if(len(trigger) != 0 and len(str_timestamp) != 0 and len(udpPacket_summary) != 0 and len(udpPacket_ecg) != 0 and len(udpPacket_breathing) != 0): 
        #     if(isSynchDone == False):
        #         synchDoneMessage = "sync done"
        #         transmitter_sock.sendto(synchDoneMessage.encode(), local_transmitterEP)
        #         isSynchDone = True
            
            
        #     if(trigger == "1"):
        #         transmitter_sock.sendto(udpPacket.encode(), local_transmitterEP)
        #         #print("Sent message: " + udpPacket) #debug
        #     elif(trigger != "1" and trigger != "0"):
        #         trigger = "0"


        # # Write on local file
        # file = open(filename_summary, "a")
        # #file.write(trigger + "\t" + str_timestamp + "\t" + udpPacket_summary + "\t" + udpPacket_ecg + '\t' + udpPacket_breathing + '\n')
        # #file.write(udpPacket_summary)
        # file.write(udpPacket)
        # file.close()
...
class ECGWaveformMessage(WaveformMessage):
    ...
    def __init__(self, msgid, payload, fin):
        ...
        # Francesco
        global udpPacket_ecg
        ecg_wave = repr(self.waveform)
        #udpPacket_ecg = trigger + '\t' + str_timestamp + '\t' + ecg_wave + "\n" # string to locally write on file
        udpPacket_ecg = ecg_wave + "\t" #string to stream

        # # Write on local file
        # file = open(filename_ecg, "a")
        # #file = open(filename_summary, "a")
        # # file.write(trigger + '\t' + str_timestamp)
        # # file.write(ecg_wave + "\n")
        # file.write(udpPacket_ecg)
        # file.close()
...
class BreathingWaveformMessage(WaveformMessage):
    ...
    def __init__(self, msgid, payload, fin):
        ...
        # Francesco
        global udpPacket_breathing
        RR_wave = repr(self.waveform)
        #udpPacket_breathing = trigger + '\t' + str_timestamp + '\t' + RR_wave + "\n" # string to locally write on file
        udpPacket_breathing = RR_wave + "\n" #string to stream

        # # Write on local file
        # file = open(filename_breathing, "a")
        # #file = open(filename_summary, "a")
        # # file.write(trigger + '\t' + str_timestamp)
        # # file.write(RR_wave + "\n")
        # file.write(udpPacket_breathing)
        # file.close()
...
def get_datetime():
    ...
    date_time = str_year + '-' + str_month + '-' + str_day + '-' + str_hour + '-' + str_minute + '-' + str_second
    return date_time


def get_file_path(save_path, current_datetime, subjID):
    local_path = os.getcwd()
    parent_dir = os.path.abspath(os.path.join(local_path, os.pardir))

    filename = os.path.join(local_path, save_path, (current_datetime + '_subj' + subjID + '.txt'))
    return filename
...
def receiver_thread():
    ...
    try:
        while True:
            data, addr = receiver_sock.recvfrom(3)
            #print ("Received message inside thread:", data.decode("utf-8"))
...
def transmitter_thread():
    ...
    try:
        while True:
            # print("lenght of trigger = " + str(len(trigger)))
            # print("length of str_timestamp = " + str(len(str_timestamp)))
            # print("length of udpPacket_summary = " + str(len(udpPacket_summary)))
            # print("length of udpPacket_ecg = " + str(len(udpPacket_ecg)))
            # print("length of udpPacket_breathing = " + str(len(udpPacket_breathing)))
            if(len(trigger) != 0 and len(str_timestamp) != 0 and len(udpPacket_summary) != 0 and len(udpPacket_ecg) != 0 and len(udpPacket_breathing) != 0):
                if(isSynchDone == False):
                    synchDoneMessage = "sync done"
                    transmitter_sock.sendto(synchDoneMessage.encode(), local_transmitterEP)
                ...
                # if(str_timestamp != temp_timestamp):
                #     udpPacket = trigger + "\t" + str_timestamp + "\t" + udpPacket_summary + udpPacket_ecg + udpPacket_breathing
                #     if(trigger == "1"):
                #         transmitter_sock.sendto(udpPacket.encode(), local_transmitterEP)
                #         #print("Sent message: " + udpPacket) #debug
                ...
                #     temp_timestamp = str_timestamp

                # # Write on local file inside if statement
                # file = open(filename_summary, "a")
                # file.write(udpPacket)
                # file.close() 
    except KeyboardInterrupt:
...
# Global variables
receiverPort = 50132 #TO DO: switch to multicast connection
transmitterPort = 50134
trigger = "0"
udpPacket = ""
udpPacket_summary = ""
udpPacket_ecg = ""
...
#subjID = input("Please enter subject's ID: ")
subjID = str(00)

current_datetime = get_datetime()
#print("Date and time: " +  current_datetime) #debug

# save_path_ecg = 'BioharnessData_ECG'
# save_path_breathing = 'BioharnessData_Breathing'
# save_path_summary = 'BioharnessData_Summary'

save_path_summary = "Bioharness Data"

# if os.path.isdir(save_path_ecg) == False:
#     os.mkdir(save_path_ecg)
# if os.path.isdir(save_path_breathing) == False:
#     os.mkdir(save_path_breathing)
if os.path.isdir(save_path_summary) == False:
    os.mkdir(save_path_summary)

# filename_ecg = os.path.join("c:", save_path_ecg, current_datetime + '_subj' + subjID + '.txt')
# filename_breathing = os.path.join("c:", save_path_breathing, current_datetime + '_subj' + subjID + '.txt')
# filename_summary = os.path.join("c:", save_path_summary, current_datetime + '_subj' + subjID + '.txt')

# filename_ecg = get_file_path(save_path_ecg, current_datetime, subjID)
# filename_breathing = get_file_path(save_path_breathing, current_datetime, subjID)
filename_summary = get_file_path(save_path_summary, current_datetime, subjID)


# Create a UDP socket to receive messages from a server and bind it to the receiver port
receiver_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
local_receiverEP = ('localhost', receiverPort) #local EndPoint used to receive messages from a remote server (trigger from CCS)
print('Listening on {} port {}'.format(*local_receiverEP))
receiver_sock.bind(local_receiverEP)

# Start a thread for the receiver side
try:
...
# Create a UDP socket to send messages to a remote server
transmitter_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
local_transmitterEP = ('localhost', transmitterPort)
print('Sending on {} port {}'.format(*local_transmitterEP))

#Debug - Francesco 20.01.22
# message = b"BioHarness is connected successfully"
# transmitter_sock.sendto(message, local_transmitterEP)
#print("UDP Packet storing message (", message, ") sent to ", local_transmitterEP)

# # Start a thread for the transmitter side
# try:
